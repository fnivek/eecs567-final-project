...
void SetupKinematics(void) {
	float zeros[3] = {0,0,0};//{0,-0.7227,1.4455};
	Point3 end_effector = KinematicsGetPosition(zeros);
	Point3ToFloatArray(end_effector, _KinematicsLastCmd);
	_KinematicsTarget 	= end_effector;
	_KinematicsEnable 	= 0;
...
void KinematicsAdvanceControl(void) {
	if (_KinematicsEnable) {
		...
		Point3ToFloatArray(KinematicsGetPosition(angles), current_x);

		ProductScalarV3(-1, current_x, current_x);
		SumV3V3(target, current_x, error);
			...
			case kInverseJacobian:
				{
					...
					float last_cmd[3];
					float d_cmd[3];
					ProductScalarV3(-1.0f, _KinematicsLastCmd, last_cmd);
					SumV3V3(target, last_cmd, d_cmd);
					ProductScalarV3(1 / kKinematicsUpdatePeriod, d_cmd, d_cmd);

					// J^-1 * (k * err + d_cmd)
					float J_inv[3][3] = {{1e-3,0,0},{0,1e-3,0},{0,0,1e-3}};
		...
		for(uint8_t i = 0; i < 3; ++i) {
			if(dq[i] > kKinematicsMaxJointChange) {
				dq[i] = kKinematicsMaxJointChange;
			} else if(dq[i] < kKinematicsMinJointChange) {
				dq[i] = kKinematicsMinJointChange;
			}
		}
		...
		// Update last command
		memcpy(_KinematicsLastCmd, target, 3 * sizeof(float));
	}
}
